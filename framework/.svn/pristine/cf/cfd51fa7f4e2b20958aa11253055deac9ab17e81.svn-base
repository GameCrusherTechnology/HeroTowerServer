...
class TCUpdateRequest extends TCRequest{
	...
	public function execute() {
		...
		$cache_commit_threshold_key = sprintf(CACHE_KEY_CACHE_COMMIT_THRESHOLD, $this->table, implode("_",$val));
		...
		if ($this->no_db) {
			...
			if ($cache_commit_count !== false && intval($cache_commit_count)+1 <= $commit_threshold) {
				$update_db = false;
			}
		} 
		if (!$update_db){
		...
		//先update cache, 再update db 
		//因为update db 比较慢，如果先update db 可能会导致cache 里面没有更新的数据被访问到
		if (!$this->no_cache) {
			$this->updateCache();
		}
		if ($update_db){
