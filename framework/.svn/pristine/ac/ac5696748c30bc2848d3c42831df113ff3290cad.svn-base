...
include_once 'xiaoiapi_restlib.php';

define('XIAOI_API_VALIDATION_ERROR', 1);
...
class Xiaoi {
  ...
  /*
   * Create a Xiaoi client like this:
   *
   * $fb = new Xiaoi(API_KEY, SECRET);
   *
   * This will automatically pull in any parameters, validate them against the
   * session signature, and chuck them in the public $xiaoi_params member variable.
   *
  ...
  public function __construct($api_key, $secret, $generate_session_secret=false) {
    ...
    $this->api_client = new XiaoiRestClient($api_key, $secret, null);

    $this->validate_xiaoi_params();
    ...
    if (isset($this->xiaoi_params['friends'])) {
      $this->api_client->friends_list = explode(',', $this->xiaoi_params['friends']);
    }
    if (isset($this->xiaoi_params['added'])) {
      $this->api_client->added = $this->xiaoi_params['added'];
    }
    if (isset($this->xiaoi_params['canvas_user'])) {
      $this->api_client->canvas_user = $this->xiaoi_params['canvas_user'];
    }
  }

  /*
   * Validates that the parameters passed in were sent from Xiaoi. It does so
   * by validating that the signature matches one that could only be generated
   * by using your application's secret key.
   *
   * Xiaoi-provided parameters will come from $_POST, $_GET, or $_COOKIE,
   ...
   * http://wiki.developers.xiaoi.com/index.php/Verifying_The_Signature
   *
   * @param bool  resolve_auth_token  convert an auth token into a session
   */
  public function validate_xiaoi_params($resolve_auth_token=true) {
    $this->xiaoi_params = $this->get_valid_xiaoi_params($_POST, 48*3600, 'xiaoi_sig');
    ...
    if (!$this->xiaoi_params) {
      $xiaoi_params = $this->get_valid_xiaoi_params($_GET, 48*3600, 'xiaoi_sig');
      $xiaoi_post_params = $this->get_valid_xiaoi_params($_POST, 48*3600, 'xiaoi_post_sig');
      $this->xiaoi_params = array_merge($xiaoi_params, $xiaoi_post_params);
    }

    // Okay, something came in via POST or GET
    if ($this->xiaoi_params) {
      $user               = isset($this->xiaoi_params['user']) ?
                            $this->xiaoi_params['user'] : null;
      $this->profile_user = isset($this->xiaoi_params['profile_user']) ?
                            $this->xiaoi_params['profile_user'] : null;
      $this->canvas_user  = isset($this->xiaoi_params['canvas_user']) ?
                            $this->xiaoi_params['canvas_user'] : null;
      $this->base_domain  = isset($this->xiaoi_params['base_domain']) ?
                            $this->xiaoi_params['base_domain'] : null;

      if (isset($this->xiaoi_params['session_key'])) {
        $session_key =  $this->xiaoi_params['session_key'];
      } else if (isset($this->xiaoi_params['profile_session_key'])) {
        $session_key =  $this->xiaoi_params['profile_session_key'];
      } else {
        $session_key = null;
      }
      $expires     = isset($this->xiaoi_params['expires']) ?
                     $this->xiaoi_params['expires'] : null;
      $this->set_user($user,
                      $session_key,
                      $expires);
    }
    // if no Xiaoi parameters were found in the GET or POST variables,
    ...
    // Cookies are also used to receive session data via the Javascript API
             ...
             $this->get_valid_xiaoi_params($_COOKIE, null, $this->api_key)) {
      ...
      // multiple xiaoi clients on the same domain.
    ...
    return !empty($this->xiaoi_params);
  }
  ...
  public function expire_session() {
    if ($this->api_client->auth_expireSession()) {
      if (!$this->in_xiaoi_canvas() && isset($_COOKIE[$this->api_key . '_user'])) {
  ...
  public function redirect($url) {
    ...
    } else if (preg_match('/^https?:\/\/([^\/]*\.)?xiaoi\.com(:\d+)?/i', $url)) {
      // make sure xiaoi.com url's load in the full frame so that we don't
  ...
  public function in_frame() {
    return isset($this->xiaoi_params['in_canvas']) || isset($this->xiaoi_params['in_iframe']);
  }
  public function in_xiaoi_canvas() {
    return isset($this->xiaoi_params['in_canvas']);
  }
  ...
  // see http://developer.xiaoi.com/news.php?blog=1&story=116 for more details
  ...
  public static function get_xiaoi_url($subdomain='www') {
    return 'http://' . $subdomain . '.xiaoi.com';
  }
  ...
  public function get_add_url($next=null) {
    return self::get_xiaoi_url().'/add.php?api_key='.$this->api_key .
      ($next ? '&next=' . urlencode($next) : '');
  }

  public function get_login_url($next, $canvas) {
    return self::get_xiaoi_url().'/login.php?v=1.0&api_key=' . $this->api_key .
      ($next ? '&next=' . urlencode($next)  : '') .
      ($canvas ? '&canvas' : '');
  }
  ...
  /*
   * Get the signed parameters that were sent from Xiaoi. Validates the set
   * of parameters against the included signature.
   *
   * Since Xiaoi sends data to your callback URL via unsecured means, the
   * signature is the only way to make sure that the data actually came from
   * Xiaoi. So if an app receives a request at the callback URL, it should
   ...
   *      www.your-callback-url.com/?xiaoi_user=10101
   *
   * This is done automatically by verify_xiaoi_params.
   *
   ...
   *                            to verify. i.e., xiaoi_sig or xiaoi_post_sig
   *
   ...
   */
  public function get_valid_xiaoi_params($params, $timeout=null, $namespace='xiaoi_sig') {
    $prefix = $namespace . '_';
   ...
   * Parameters all match a given input prefix, such as "xiaoi_sig".
   *
   * @param $xiaoi_params     an array of all Xiaoi-sent parameters,
   *                       not including the signature itself
   * @param $expected_sig  the expected result to check against
   */
  public function verify_signature($xiaoi_params, $expected_sig) {
    return self::generate_sig($xiaoi_params, $this->secret) == $expected_sig;
  }

  /*
   * Generate a signature using the application secret key.
   *
   * The only two entities that know your secret key are you and Xiaoi,
   ...
   * came from Xiaoi.
   *
   * @param $params_array   an array of all Xiaoi-sent parameters,
   *                        NOT INCLUDING the signature itself
   * @param $secret         your app's secret key
   *
   * @return a hash to be checked against the signature provided by Xiaoi
   */
  ...
  public function encode_validationError($summary, $message) {
    return json_encode(
               array('errorCode'    => XIAOI_API_VALIDATION_ERROR,
                     'errorTitle'   => $summary,
