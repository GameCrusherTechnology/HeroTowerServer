...
class DBHelper2 {
	...
	private function getConnection($re_select_db=true){
		if(!isset($this->conn)){
			...
			if ($this->logger->isDebugEnabled()) {
				$this->logger->writeDebug(
					"[db_helper=%s]tring to connect to database[host=%s,username=%s,database=%s,pconnect=%s,charset=%s,newlink=%s]",
					array(
		...
		if ($re_select_db) $this->conn->selectDB($this->config['database'], $this->config);
		return $this->conn;
	}
